...
legislatives <- read_rds("data/election_results_candidates_by_constituency.rds")

## Circonscriptions non affichées sur la carte et résultats 1er tour
legislatives_empty <- read_rds("data/unmapped_constituencies_first_round.rds")

## Villes et coordonnées géographiques
villes <- read_rds("data/cities_coordinates.rds")

## Circonscriptions non affichées sur la carte et résultats 1er et 2nd tour (candidats 20 à 23)
legislatives_empty2 <- read_rds("data/unmapped_constituencies_all_rounds.rds")
                                              ...
                                              tagList(
                                                ...
                                                actionButton(
                                                  ...
                                                  label = HTML(paste0(
                                                    ...
                                                    "<img src='assets/", gsub(" ", "_", ordre_partis2), ".png' ",
...
server <- function(input, output, session) {
  ...
  lapply(ordre_partis2, function(parti) {
    output[[paste0("download_", gsub(" ", "_", parti))]] <- downloadHandler(
      filename = function() {
        slug <- to_slug(parti)
        paste0(slug, "_programme.pdf")  # => ex: lutte_ouvriere_programme.pdf
      },
      content = function(file) {
        slug <- to_slug(parti)
        src  <- file.path("www", "references", paste0(slug, "_programme.pdf"))
